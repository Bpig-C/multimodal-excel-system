...
backend/migrations/add_content_hash.sql
...
## File: backend/migrations/add_content_hash.sql
````sql
-- 迁移：为 KF/QMS 主事实表添加 content_hash 列
-- 适用数据库：SQLite
-- 执行方式：sqlite3 <db_path> < add_content_hash.sql
--
-- SQLite 不支持 ADD COLUMN + UNIQUE 约束同时完成，
-- 先加列（允许 NULL），再建唯一索引。

ALTER TABLE quick_response_events ADD COLUMN content_hash VARCHAR(64);
CREATE UNIQUE INDEX IF NOT EXISTS uix_qre_content_hash
    ON quick_response_events (content_hash)
    WHERE content_hash IS NOT NULL;

ALTER TABLE qms_defect_orders ADD COLUMN content_hash VARCHAR(64);
CREATE UNIQUE INDEX IF NOT EXISTS uix_qdo_content_hash
    ON qms_defect_orders (content_hash)
    WHERE content_hash IS NOT NULL;
````
