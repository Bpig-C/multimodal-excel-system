...
<template>
  <div class="corpus-preview">
    ...
    <div class="filter-toolbar">
      ...
      <el-select
        v-model="selectedType"
        placeholder="鎸夊瓧娈靛垎绫荤瓫閫?
        clearable
        style="width: 200px"
        @change="handleFilterChange"
      >
        <el-option label="鍏ㄩ儴" value="" />
        <el-option label="闂鎻忚堪" value="闂鎻忚堪" />
        <el-option label="鍘熷洜鍒嗘瀽" value="鍘熷洜鍒嗘瀽" />
        <el-option label="閲囧彇鎺柦" value="閲囧彇鎺柦" />
      </el-select>
      ...
      <div class="stats">
        <span>鍏?{{ total }} 鏉¤鏂?/span>
        <span v-if="filteredCount !== total">锛堢瓫閫夊悗 {{ filteredCount }} 鏉★級</span>
      </div>
    </div>

    <!-- 璇枡鍒楄〃 -->
    <div v-loading="loading" class="corpus-list">
      <el-empty v-if="!loading && list.length === 0" description="鏆傛棤璇枡鏁版嵁" />
      ...
      >
        <div class="corpus-header">
          ...
          <div class="corpus-meta">
            ...
            <span class="meta-item">
              绗?{{ item.source_row }} 琛?            </span>
            <span v-if="item.has_images" class="meta-item">
              <el-icon><Picture /></el-icon>
              {{ item.images?.length || 0 }} 寮犲浘鐗?            </span>
          </div>
        </div>
...
// 璁＄畻灞炴€?const loading = computed(() => corpusStore.loading)
...
const handleView = (item: Corpus) => {
  // TODO: 璺宠浆鍒拌鎯呴〉鎴栨墦寮€璇︽儏瀵硅瘽妗?  console.log('鏌ョ湅璇︽儏:', item)
}

const handleDelete = async (item: Corpus) => {
  try {
    await ElMessageBox.confirm(
      `纭畾瑕佸垹闄よ鏂?${item.text_id} 鍚楋紵姝ゆ搷浣滀笉鍙仮澶嶃€俙,
      '鍒犻櫎纭',
      {
    ...
    // 濡傛灉褰撳墠椤垫病鏈夋暟鎹簡锛岃繑鍥炰笂涓€椤?    if (list.value.length === 0 && currentPage.value > 1) {
      currentPage.value--
    }
    ...
    if (error !== 'cancel') {
      const msg = error?.response?.data?.detail || '鍒犻櫎澶辫触'
      ElMessage.warning(msg)
    }
  }
}
